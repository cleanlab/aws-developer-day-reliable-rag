...
class RAG:
    ...
    def query(self, question: str) -> Response:
        ...
        validation_results = self._validator.validate(
            query=question, context=context, response=response, form_prompt=self._format_prompt
        )
        is_bad_response, _, eval_results = self._parse_validation_results(validation_results)

        return {
            "response": response,
            "is_bad_response": is_bad_response,
            "is_expert_answer": False, # not implemented in part 2
            "evals": eval_results,
        }
